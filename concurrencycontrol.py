...
Original file is located at
    https://colab.research.google.com/drive/17rEoCQKBCQbvYHuzBF6iACIZ3In_h2X9
"""
...
def serialTransactions():
  ...
  for transId in range(1,7):# each transaction
    filename='sample_data/inputs/t'+str(transId)+'.txt'
    ...
    for ind,line in enumerate(Lines):
      ...
      elif(c.startswith('WRITE',0,5)): #WRITE  
        numsInd=operands[2]
        if operands[2][0]=='A':
      ...
      elif(c.startswith('EXECUTE',0,7)):#EXECUTE
        if '=' in operands[1]:
      ...
      elif(c.startswith('COMMIT',0,6)):
        log=operands[0]+' transaction T'+str(transId)+'\n'
        schedulerFile.write(log)


      elif(c.startswith('END',0,6)):
        log=operands[0]+' transaction T'+str(transId)+'\n'
        schedulerFile.write(log)
    file.close()
    schedulerFile.close()
